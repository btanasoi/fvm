...
def dumpvtkEntireThermalDomain2D(geomFields, nmesh, meshesLocal, meshesGlobal, mtype, tFields,nstep):
  ...
  temperatureFields = []
  for n in range(0,nmesh):
     temperatureFields.append( tFields.temperature[cellSitesLocal[n]].asNumPyArray() )    

  temperaturegradFields = []
  for n in range(0,nmesh):
     temperaturegradFields.append( tFields.temperatureGradient[cellSitesLocal[n]].asNumPyArray() )

  #opening global Array 
  temperatureFieldGlobal = []
  temperaturegradFieldGlobal = []

  for n in range(0,nmesh):

    selfCount = cellSites[n].getSelfCount()
    temperatureFieldGlobal.append( zeros((selfCount,1), float) ) #if it is velocoity (selfCount,3) 
    temperaturegradFieldGlobal.append( zeros((selfCount,3), float) ) #if it is velocity (selfCount,3)
    meshLocal = meshesLocal[n]
    localToGlobal = meshLocal.getLocalToGlobalPtr().asNumPyArray()
    tFieldGlobal  = temperatureFieldGlobal[n]
    tgradFieldGlobal  = temperaturegradFieldGlobal[n]  
    tFieldLocal   = temperatureFields[n]
    tgradFieldLocal   = temperaturegradFields[n]
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
     ...
     file_name_vtk = "temperature-"+str(nstep)+".vtk"
