...
timeStep = 1e-8                  # Size of timestep (seconds)
numTimeSteps = 4000               # Number of timesteps
...
soptions.timeStepN1 = timeStepN1
soptions.timeStepN2 = timeStepN2
...
for n in range(0,nmesh):
    densityFields = structureFields.density[cellSitesLocal[n]]
    densityFieldsA = densityFields.asNumPyArray() 
...
################################################################## Particles
        ...
        (localToGlobal[i] >= 1941 and localToGlobal[i]< 12304):
            ...
            densityFieldsA[i] = density_fiber
...
################################################################## Particles        
            ...
            if particle_interface_distance_min<0.008e-3:
                ...
                E_local[i]=E_fiber*(E/E_fiber)**((particle_interface_distance_min-0.008e-3)/(0.018e-3-0.008e-3))
                nu_local[i]=nu_fiber*(nu/nu_fiber)**((particle_interface_distance_min-0.008e-3)/(0.018e-3-0.008e-3))
                densityFieldsA[i]=density_fiber*(density/density_fiber)**((particle_interface_distance_min-0.008e-3)/(0.018e-3-0.008e-3))
