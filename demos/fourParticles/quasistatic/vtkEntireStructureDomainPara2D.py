...
def dumpvtkEntireStructureDomain(geomFields, nmesh, meshesLocal, meshesGlobal, mtype, sFields, file_name, title_name,nstep):
  ...
  etaFields = []
  for n in range(0,nmesh):
     etaFields.append( sFields.eta[cellSitesLocal[n]].asNumPyArray() )  
  ...
  for n in range(0,nmesh):
    ...
    etaFieldGlobal.append( zeros((selfCount,1), float) ) #if it is velocoity (selfCount,3) 
    #J1 and J2 Field
    ...
    etaFieldGlobal  = etaFieldGlobal[n]    
    #J1 and J2 Field
    ...
    for i in range(0,selfCount):
       ...
       etaFieldGlobal[globalID] = etaFieldLocal[i]
       #J1 and J2 Field
    ...
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [etaFieldGlobal,MPI.DOUBLE], op=MPI.SUM)
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
     ...
     f.write("SCALARS "+"etaField"+" double\n")
     f.write("LOOKUP_TABLE default\n")
     for i in range(0,ncell):
         f.write(str(etaFieldGlobal[i][0])+"\n") 
