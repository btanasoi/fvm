...
def dumpvtkEntireFractureDomain(geomFields, nmesh, meshesLocal, meshesGlobal, mtype, tFields, sFields, file_name, title_name,nstep, fractureToughnessField):
  ...
  for n in range(0,nmesh):
    ...
    fractureToughnessFieldGlobal.append( zeros((selfCount,1), float) ) #if it is velocoity (selfCount,3) 
    ...
    for i in range(0,selfCount):
       ...
       ftFieldGlobal[globalID] = ftFieldLocal[i]
    ...
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE, [ftFieldGlobal,MPI.DOUBLE], op=MPI.SUM)
  ...
  if MPI.COMM_WORLD.Get_rank() == 0:
     ...
     f.write("SCALARS "+"fractureToughnessField"+" double\n")
     f.write("LOOKUP_TABLE default\n")
     for i in range(0,ncell):
         f.write(str(ftFieldGlobal[i][0])+"\n") 
